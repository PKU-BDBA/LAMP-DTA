...
class LAMPDatasetRaw(Dataset):
    ...
    def __getitem__(self, idx):
        row = self.data.loc[idx]
        return torch.FloatTensor([row.affinity]), row.compound_iso_smiles, row.prot_key, row.protein_id
    

@dataclass
class LAMPDatasetCM(Dataset):
    dataset: str
    stage: str
    fold: str
    ...
    def __post_init__(self):
        if self.stage == 'test':
            data_path = f'data/{self.dataset}/{self.dataset}_test_cm.csv'
        else:
            data_path = f'data/{self.dataset}/{self.dataset}_{self.stage}_fold_{self.fold}_cm.csv'
        data = pd.read_csv(data_path, low_memory=False)

        self.data = data[~data['affinity'].isna()]

        self.pid2llm = np.load(f'{self.dataset}.npz',allow_pickle=True)['dict'][()]
        cm_data_path = '/data5/luozc/projects/DTA/LAMP-DTA/GINCM-DTA/data/'
        self.contact_path = cm_data_path + self.dataset + '/pconsc4'
        self.msa_path = cm_data_path + self.dataset + '/aln'
    ...
    def __getitem__(self, idx):
        row = self.data.loc[idx]
        label, cis, pid, key, seq = (
            row.affinity,
            row.compound_iso_smiles,
            row.protein_id,
            row.target_original_key,
            row.target_sequence
        )
        c_size, feats, edge_index = smile_to_graph(cis)
        drug_g = DATA.Data(x=torch.Tensor(np.array(feats)),
        ...
        tar_size, tar_feats, tar_edge_index = target_to_graph(key, seq, self.contact_path, self.msa_path)
        prot_g = DATA.Data(x=torch.Tensor(np.array(tar_feats)),
                                edge_index=torch.LongTensor(tar_edge_index).transpose(1, 0))
        prot_g.__setitem__('target_size', torch.LongTensor([tar_size]))

        return torch.FloatTensor([label]), drug_g, prot_g, self.pid_to_llm(pid)