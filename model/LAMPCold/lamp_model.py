...
class LAMPCM(nn.Module): # TODO
    def __init__(self, config):
        ...
        self.mol_conv2 = GINConv(nn2)
        self.mol_conv3 = GINConv(nn3)
        # self.mol_fc1 = nn.Linear(config.num_features_mol * 4, config.num_features_mol * 64)
        self.mol_fc2 = nn.Linear(config.num_features_mol * 4, config.embed_dim * config.num_features_mol)

        # prot_g
        self.embed_dim = config.embed_dim
        pro_nn1 = Sequential(Linear(config.num_features_prot, config.num_features_prot), GELU(), Linear(config.num_features_prot, config.num_features_prot))
        pro_nn2 = Sequential(Linear(config.num_features_prot, config.num_features_prot*2), GELU(), Linear(config.num_features_prot*2, config.num_features_prot*2))
        pro_nn3 = Sequential(Linear(config.num_features_prot*2, config.num_features_prot*4), GELU(), Linear(config.num_features_prot*4, config.num_features_prot*4))
        self.pro_conv1 = GINConv(pro_nn1)
        self.pro_conv2 = GINConv(pro_nn2)
        self.pro_conv3 = GINConv(pro_nn3)
        # self.pro_fc1 = nn.Linear(config.num_features_prot * 4, config.num_features_prot * 64)
        self.pro_fc2 = nn.Linear(config.num_features_prot * 4, config.embed_dim * config.num_features_prot)

        # prot_llm
        self.llm_fc = nn.Linear(1024, 64)
        self.llm_embed = nn.Linear(2560, CONFIGS['PMMA'].hidden_size // 2)
        self.prot_llm_bilstm = nn.LSTM(CONFIGS['PMMA'].hidden_size // 2, CONFIGS['PMMA'].hidden_size // 2, num_layers=2, bidirectional=True, dropout=config.dropout)
    ...
    def extract_mol_feat(self, mol_g):
        ...
        # mol_feat = self.dropout(self.gelu(self.mol_fc1(mol_feat)))
        mol_feat = self.dropout(self.mol_fc2(mol_feat))
        mol_feat = mol_feat.view(-1, mol_feat.shape[0], self.embed_dim)

        return mol_feat
    
    def extract_prot_embed(self, prot_g):
        prot_x, prot_edge_index, prot_batch = prot_g.x, prot_g.edge_index, prot_g.batch
        
        prot_embed = self.gelu(self.pro_conv1(prot_x, prot_edge_index))
        prot_embed = self.gelu(self.pro_conv2(prot_embed, prot_edge_index))
        prot_embed = self.gelu(self.pro_conv3(prot_embed, prot_edge_index))

        prot_embed = gep(prot_embed, prot_batch)

        # prot_embed = self.dropout(self.gelu(self.pro_fc1(prot_embed)))
        prot_embed = self.dropout(self.pro_fc2(prot_embed))
        prot_embed = prot_embed.view(-1, prot_embed.shape[0], self.embed_dim)
    ...
    def forward(self, mol_g, prot_g, prot_llms):
        prot_embed = self.extract_prot_embed(prot_g)
        mol_feat = self.extract_mol_feat(mol_g)
        prot_llms = self.fill_prot_llms(prot_llms)
        
        mol_embed, self.A_tgca = self.tgca(prot_llms.permute(1, 0, 2), mol_feat, mol_feat)
