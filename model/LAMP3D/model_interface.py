...
class MInterface(pl.LightningModule):
    ...
    def setup_fold_index(self, fold_index: int, is_test: bool=False):
        ...
        if self.global_rank == 0:
            self.printed += f"task={self.config.model}_{self.config.dataset}, fold={fold_index}" + '\n'
    ...
    def on_train_start(self) -> None:
        if self.log_path is None:
            log_dir = self.log_dir / f'fold_{self.fold}'
    ...
    def configure_optimizers(self):
        ...
        scheduler = CosineAnnealingWarmupRestarts(
            ...
            max_lr=self.lr,
            min_lr=1e-8,
            warmup_steps=int(self.num_train_epochs * 0.03)
        )
    ...
    def load_model(self):
        try:
            self.Model = getattr(importlib.import_module(
                '.lamp_model', package=__package__), 'LAMP')
        except:
            raise ValueError(
