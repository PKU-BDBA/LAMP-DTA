...
def train_eval(
    ...
    for epoch in range(epochs):
        ...
        print('Epoch', epoch)
        train(model, device, train_loader, optimizer, epoch + 1, pid2llm=pid2llm)
        print('predicting for valid data')
        G, P = predicting(model, device, valid_loader, pid2llm=pid2llm)
        val1 = get_mse(G, P)
        if test_loader is not None:
            print('predicting for test data')
            G, P = predicting(model, device, test_loader, pid2llm=pid2llm)
...
def train_eval_new(
    ...
    # pid2llm = np.load(f'../{dataset}.npz',allow_pickle=True)['dict'][()]
    pid2llm_dir = f'../data/{dataset}/llm/'
    for epoch in range(epochs):
        endtime = datetime.datetime.now()
        print('total run time: ', endtime - starttime)
        print('last epoch run time: ', endtime - last_epoch_time)
        last_epoch_time = endtime
        print('Epoch', epoch)
        train_new(model, device, train_loader, optimizer, epoch + 1, pid2llm_dir=pid2llm_dir)
        print('predicting for valid data')
        G, P = predicting_new(model, device, valid_loader, pid2llm_dir=pid2llm_dir)
        val1 = get_mse(G, P)
        if test_loader is not None:
            print('predicting for test data')
            G, P = predicting_new(model, device, test_loader, pid2llm_dir=pid2llm_dir)
...
def train_eval_reduced(
    ...
    pid2llm = np.load(f'../{dataset}_reduced.npz',allow_pickle=True)['dict'][()]
    for epoch in range(epochs):
        endtime = datetime.datetime.now()
        print('total run time: ', endtime - starttime)
        print('last epoch run time: ', endtime - last_epoch_time)
