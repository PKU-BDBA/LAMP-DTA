...
def valid_target(key, dataset):
    contact_dir = '/data5/luozc/projects/DTA/LAMP-DTA/GINCM-DTA/data/' + dataset + '/pconsc4'
    aln_dir = '/data5/luozc/projects/DTA/LAMP-DTA/GINCM-DTA/data/' + dataset + '/aln'
...
def pids_to_llm(pids, pid2llm, device):
    pid_list = pids.tolist()
    return [torch.from_numpy(pid2llm[pid]).squeeze().to(device) for pid in pid_list]
    
def train(model, device, train_loader, optimizer, epoch, verbose=True, pid2llm=None): # XXX Use logger
...
# predict
def predicting(model, device, loader, pid2llm):
    model.eval()
...
def pids_to_llm_new(pids, pid2llm_dir, device):
    pid_list = pids.tolist()
    return [torch.from_numpy(np.load(pid2llm_dir + str(pid) + '.npy',allow_pickle=True)).squeeze().to(device) for pid in pid_list]
    
def train_new(model, device, train_loader, optimizer, epoch, verbose=True, pid2llm_dir=None): # XXX Use logger
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        data_mol = data[0].to(device)
        data_pro = data[1].to(device)
        data_llm = pids_to_llm_new(data[2], pid2llm_dir, device)
        optimizer.zero_grad()
        output = model(data_mol, data_pro, data_llm)
        loss = loss_fn(output, data_mol.y.view(-1, 1).float().to(device))
        loss.backward()
...
def predicting_new(model, device, loader, pid2llm_dir):
    model.eval()
    total_preds = torch.Tensor()
    total_labels = torch.Tensor()
    print('Make prediction for {} samples...'.format(len(loader.dataset)))
...
def collate(data_list):
    ...
    pids = [data[2] for data in data_list]
    return batchA, batchB, pids
